function fillCFWalls3b(th, th2)

%This functions takes a *.model file as input and returns a *.model file
%with all walls touching floors and ceilings

%th- distance threshold between a celilng/floor and a wall (in Z direction)
%th2 -distance threshold to smooth edges of ceilings/floors following walls
%Some vertices of celings/floors are replaced by two vertices of an
%adjecent wall
...
for wall=1:1:size(walls,1)
    ...
    for p=1:1:size(CF,1)
       ...
       %Calculate range of wall in X and Y
        ...
        if (maxZ<=(atZ+th) && maxZ>=(atZ-th)) %&& matchedCeiling==0 % wall matched with a ceiling
            matchedCeiling=1;
            %wall
