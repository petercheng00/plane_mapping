classdef plane < handle
    ...
    methods
        ...
        function obj = split_plane_texturing(obj, step, blend, maxCacheAngle, maxAngle)
            ...
            for col_min = 1:step:obj.width-step
                ...
                for row_min = 1:step:obj.height-step
                    ...
                    newSection.row_min = max(1,row_min-blend);
                    newSection.row_max = min(obj.height,row_max+blend);
                    newSection.col_min = max(1,col_min-blend);
                    newSection.col_max = min(obj.width,col_max+blend);
        ...
        function MST_prim(obj)
            ...
            nodes = [];
            for i = 1:10:obj.height
                for j = 1:10:obj.width
                    currNode.row = i;
                    currNode.col = j;
                    currNode.images = [];
                    currNode.covered = false;
                    for im = 1:size(obj.images,2)
                        ...
                        if dataY >= 1 && dataY <= (box.row_max-box.row_min+1) && ...
                           dataX >= 1 && dataX <= (box.col_max-box.col_min+1) && ...
                            ...
                            currNode.images = [currNode.images, im];
                            obj.images(im).nodes = [obj.images(im).nodes, currNode];
                        end
                    end
